...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
	    ...
	    String cmd = args[0];               // параметр команда
	    Path archive = Paths.get(args[1]);    // target директория
	    Path directory = Paths.get(args[2]);  // source директория
    ...
    private static void pack(Path archive, Path directory) throws IOException {
        try (YetAnotherArchiveWriter archiveWriter = new YetAnotherArchiveWriter()) {
          archiveWriter.addDirectoryRecursively(directory);
      }
    }
    private static void unpack(Path archive, Path directory) throws  IOException {
        try (YetAnotherArchiveReader archiveReader = new YetAnotherArchiveReader()) {
